...
import           Boris.Core.Data (Project (..), Build (..), Commit (..), CommitUrl (..))
import           Boris.Core.Data (Ref (..), BuildId (..), BuildResult (..), sortBuildIds)
...
build l b =
  let
    context = [
      ...
      , ("commiturl", maybe BMXNull (BMXString . renderCommitUrl) . buildDataCommitUrl $ b)
